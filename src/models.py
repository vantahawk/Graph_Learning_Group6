...
class GCN_graph(th.nn.Module):
    ...
    def __init__(self, input_dim: int, output_dim: int, dropout_list: list[int], hidden_dim: int = 64, n_GCN_layers: int = 5):
        ...
        self.hidden_MLP_layer = Linear(hidden_dim, hidden_dim, True)  # bias=True
        self.output_layer = Linear(hidden_dim, output_dim, True)  # bias=True
...
class GCN_node(th.nn.Module):
    ...
    def __init__(self, input_dim: int, output_dim: int, dropout_list: list[int], hidden_dim: int = 64, n_GCN_layers: int = 3):
...
#if __name__ == "__main__":  # optional: Ex.3/4 demo
