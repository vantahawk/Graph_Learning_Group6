...
def custom_collate(sparse_rep_list: list[tuple[th.Tensor, th.Tensor, th.Tensor, th.Tensor]]) -> tuple[th.Tensor, th.Tensor, th.Tensor, th.Tensor, th.Tensor]:
    ...
    return th.cat(edge_idx_col, -1).type(th.long), th.cat(node_features_col, 0).type(th.float), th.cat(edge_features_col, 0).type(th.float), th.tensor(graph_labels_col).type(th.float), th.cat(batch_idx, 0).type(th.long)
