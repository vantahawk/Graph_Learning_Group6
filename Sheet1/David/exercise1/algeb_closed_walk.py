...
#works fine for NCI1 & ENZYMES for max_length <= 50 and possibly higher
#in general, computation time should scale roughly quadratically w. max_length
#results may become too large (overflow, instable, etc.) for exceedingly large max_length
...
def algeb_closed_walk_kernel(graph: nx.Graph, max_length: int) -> np.ndarray:
    ...
    #Eigenvalues = eigsh(csr_matrix.asfptype(A), k=nx.number_of_nodes(Graph)-1, which='LM', maxiter=None, tol=0, return_eigenvectors=False, mode='normal') #sparse method
    Eigenvalues = eigvalsh(A.todense(), overwrite_a=False, check_finite=True, subset_by_index=None, driver=None) #dense method, 
    #eigvalsh seems to use CPU-cores most efficiently, also slightly more accurate (based on variance w.r.t. brute result)
    return np.array([np.sum(np.power(Eigenvalues, l)) for l in range(2, max_length+1)]) #returns kernel vector for walk lengths l=2 to max_length